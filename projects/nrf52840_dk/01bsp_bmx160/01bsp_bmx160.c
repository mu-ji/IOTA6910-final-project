...
//#define BUFFER_SIZE     0x0E   //2B*3 gyr + 2B*3 acc + 2B ending with '\r\n'
#define BUFFER_SIZE     0x14   //2B*3 gyr + 2B*3 acc + 2B ending with '\r\n'
//=========================== variables =======================================
...
int mote_main(void) {
    ...
    while (1) {
        ...
        //tmp = bmx160_read_mag_x();
        //app_vars.uartToSend[i++] = (uint8_t)((tmp>>8) & 0x00ff);
        //app_vars.uartToSend[i++] = (uint8_t)((tmp>>0) & 0x00ff);

        //tmp = bmx160_read_mag_y();
        //app_vars.uartToSend[i++] = (uint8_t)((tmp>>8) & 0x00ff);
        //app_vars.uartToSend[i++] = (uint8_t)((tmp>>0) & 0x00ff);

        //tmp = bmx160_read_mag_z();
        //app_vars.uartToSend[i++] = (uint8_t)((tmp>>8) & 0x00ff);
        //app_vars.uartToSend[i++] = (uint8_t)((tmp>>0) & 0x00ff);
...
void cb_uartTxDone(void) {
    ...
    if (app_vars.uart_lastTxByteIndex<14) {
        uart_writeByte(app_vars.uartToSend[app_vars.uart_lastTxByteIndex]);
    } else {
        app_vars.uartDone = 1;
    }
